...
class WeatherEndpointWithPOSTTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.maxDiff = None
        self.invalid_chicago_message = ["A valid number is required."]

    def test_with_valid_data(self):
        r = self.client.post(self.url, json=chicago)
    ...
    def test_with_invalid_data(self):
        r = self.client.post(self.url, json=invalid_chicago)
        data = r.json()
        self.assertEqual(r.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertListEqual(data['lat'], self.invalid_chicago_message)
...
class WeatherEndpointWithGETSingleTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = HOST + '/weather/'
        try:
            self.chicago = self.client.post(self.url, json=chicago).json()
        except JSONDecodeError:
            self.fail("/weather endpoint for POST request not implemented")

    def test_with_existing_record(self):
        url = self.url + ('%s' % self.chicago['id'])
        r = self.client.get(url)
        data = r.json()
        self.assertEqual(data['id'], self.chicago['id'])
        self.assertEqual(r.status_code, status.HTTP_200_OK)
        self.assertDictEqual(self.chicago, data)

    def test_with_non_existing_record(self):
        non_existing_id = 9999
        url = self.url + ('%s' % non_existing_id)
        r = self.client.get(url)
...
class WeatherEndpointWithGETListTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.url = HOST + '/weather/'
        try:
            self.objects = [self.client.post(self.url, json=city).json()
